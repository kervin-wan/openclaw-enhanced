...
// Enhanced: read thinking.conf for status display
let _thinkingConfPath = null;
function getThinkingConfPath() {
  if (!_thinkingConfPath) _thinkingConfPath = path.join(os.homedir(), '.openclaw-enhanced', 'thinking.conf');
  return _thinkingConfPath;
}
function readThinkingConf() {
  try { return fs.readFileSync(getThinkingConfPath(), 'utf-8').trim(); }
  catch (_) { return 'off'; }
}
import { resolveSandboxRuntimeStatus } from "../agents/sandbox.js";
import { derivePromptTokens, normalizeUsage } from "../agents/usage.js";
import { resolveMainSessionKey, resolveFreshSessionTotalTokens, resolveSessionFilePath, } from "../config/sessions.js";
...
const readUsageFromSessionLog = (sessionId, sessionEntry, agentId, sessionKey, storePath) => {
    ...
    try {
        ...
        logPath = resolveSessionFilePath(sessionId, sessionEntry, 
...
export function buildStatusMessage(args) {
    ...
    const confMode = readThinkingConf();
    let displayThinkLevel;
    if (confMode === 'manual') {
      displayThinkLevel = 'high';
    } else if (confMode === 'off') {
      displayThinkLevel = 'off';
    } else {
      // auto: read actual think level from get-reply-run output
      try {
        displayThinkLevel = fs.readFileSync('/tmp/think-state.log', 'utf-8').trim() || 'off';
      } catch (_) {
        displayThinkLevel = args.resolvedThink ?? args.agent?.thinkingDefault ?? 'off';
      }
    }
    const thinkLevel = displayThinkLevel;
