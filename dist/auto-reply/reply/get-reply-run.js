...
export async function runPreparedReply(params) {
    ...
    // Enhanced: early auto-detect for thinking notification
    if (!resolvedThinkLevel) {
        try {
            const { createRequire: crEarly } = await import('node:module');
            const requireEarly = crEarly(import.meta.url);
            const fsEarly = requireEarly('fs');
            const pmEarly = requireEarly('path');
            const omEarly = requireEarly('os');
            const confP = pmEarly.join(omEarly.homedir(), '.openclaw-enhanced', 'thinking.conf');
            const confM = fsEarly.existsSync(confP) ? fsEarly.readFileSync(confP, 'utf-8').trim() : 'off';
            if (confM === 'auto' && rawBodyTrimmed) {
                const tmEarly = requireEarly('../../engine/ThinkingManager.cjs');
                const det = tmEarly.detectComplexity(rawBodyTrimmed);
                if (det.complex) {
                    resolvedThinkLevel = 'high';
                    const { enqueueSystemEvent } = await import('../../infra/system-events.js');
                    enqueueSystemEvent('🧠 检测到复杂任务（' + (det.reason || 'score=' + det.score) + '），已自动开启深度思考模式', { sessionKey });
                }
            } else if (confM === 'manual') {
                resolvedThinkLevel = 'high';
            } else if (confM === 'off') {
                resolvedThinkLevel = 'off';
            }
            try { fsEarly.writeFileSync('/tmp/think-state.log', String(resolvedThinkLevel) + '\n', 'utf-8'); } catch (_) {}
        } catch (_) {}
    }
    ...
    // Enhanced: ThinkingManager conf override (highest priority, includes auto-detect)
    try {
        ...
        const confPath = pathMod.join(osMod.homedir(), '.openclaw-enhanced', 'thinking.conf');
        const mode = fs.existsSync(confPath) ? fs.readFileSync(confPath, 'utf-8').trim() : 'off';
        fs.appendFileSync('/tmp/think-debug.log', new Date().toISOString() + ' ENTERED: confMode=' + mode + ' currentThink=' + resolvedThinkLevel + '\n');
        if (mode === 'manual') resolvedThinkLevel = 'high';
        else if (mode === 'auto') {
            // Run complexity detection on the user message
            const userMsg = ctx.CommandBody ?? ctx.RawBody ?? ctx.Body ?? '';
            const tm = require('../../engine/ThinkingManager.cjs');
            const detection = tm.detectComplexity(userMsg);
            fs.appendFileSync('/tmp/think-debug.log', new Date().toISOString() + ' AUTO: msg=' + JSON.stringify(userMsg.substring(0,50)) + ' complex=' + detection.complex + ' score=' + detection.score + '\n');
            if (detection.complex) {
                resolvedThinkLevel = 'high';
                // Enhanced: notify user about auto-detected thinking
                const { enqueueSystemEvent } = await import('../../infra/system-events.js');
                enqueueSystemEvent('🧠 检测到复杂任务，已自动开启深度思考模式', { sessionKey });
            }
        }
        else if (mode === 'off') resolvedThinkLevel = 'off';
        try { fs.writeFileSync('/tmp/think-state.log', String(resolvedThinkLevel) + '\n', 'utf-8'); } catch (_) {}
    } catch (_) {}
