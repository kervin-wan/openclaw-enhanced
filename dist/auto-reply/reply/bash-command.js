...
function formatElevatedUnavailableMessage(params) {
    ...
    if (params.failures.length > 0) {
        ...
        lines.push("失败的门控: enabled (tools.elevated.enabled / agents.list[].tools.elevated.enabled), allowFrom (tools.elevated.allowFrom.<provider>)。");
    }
    lines.push("修复密钥:");
    ...
    if (params.sessionKey) {
        lines.push(`查看: ${formatCliCommand(`openclaw-enhanced sandbox explain --session ${params.sessionKey}`)}`);
    }
    return lines.join("\n");
}
