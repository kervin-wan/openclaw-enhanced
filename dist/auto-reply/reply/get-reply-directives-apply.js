...
// Enhanced: ThinkingManager — reads thinking.conf for thinkLevel override
async function resolveThinkingOverride() {
  try {
    ...
    const os = require('os');
    const confPath = pathMod.join(os.homedir(), '.openclaw-enhanced', 'thinking.conf');
    const mode = fs.existsSync(confPath) ? fs.readFileSync(confPath, 'utf-8').trim() : 'off';
    if (mode === 'manual') return 'high';
...
export async function applyInlineDirectiveOverrides(params) {
    ...
    if (isDirectiveOnly({
        ...
        const thinkingOverride = await resolveThinkingOverride();
        const resolvedDefaultThinkLevel = thinkingOverride ??
