...
import { loadSessionStore, resolveAgentIdFromSessionKey, resolveMainSessionKey, resolveSessionFilePath, resolveStorePath, } from "../config/sessions.js";
...
export async function runSubagentAnnounceFlow(params) {
    ...
    try {
        ...
        // Retry up to 3 times with backoff to handle TOCTOU races when
        // multiple sub-agents complete simultaneously and a sibling
        // announcement locks the requester session before our send lands.
        ...
        if (sendOk) {
            ...
            // All retries exhausted — fall back to queue so the message isn't lost.
            // The queue will drain when the requester session becomes idle.
            const fallbackQueued = await maybeQueueSubagentAnnounce({
                requesterSessionKey: params.requesterSessionKey,
                triggerMessage,
                summaryLine: taskLabel,
                requesterOrigin,
            });
            if (fallbackQueued !== "none") {
                didAnnounce = true;
                return true;
            }
            // Queue fallback also failed — log and let registry retry on restart.
            defaultRuntime.error?.(
                `Subagent announce failed for "${taskLabel}" after 3 retries: ${lastError ? String(lastError) : "unknown"}`,
            );
        }
    }
    catch (err) {
