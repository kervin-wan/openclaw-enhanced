...
import { parseCameraClipPayload, parseCameraSnapPayload, } from "../../cli/nodes-camera.js";
                ...
                switch (action) {
                    ...
                    case "camera_snap": {
                        ...
                        for (const facing of facings) {
                            ...
                            const mimeType = imageMimeFromFormat(payload.format) ?? (isJpeg ? "image/jpeg" : "image/png");
                            const saved = await saveMediaBuffer(buffer, mimeType, "nodes", Math.max(buffer.length, 1), `camera-${facing}.${isJpeg ? "jpg" : "png"}`);
                    ...
                    case "camera_clip": {
                        ...
                        const saved = await saveMediaBuffer(buffer, contentType, "nodes", Math.max(buffer.length, 1), `camera-clip-${facing}.${format || "mp4"}`);
                        const filePath = saved.path;
                        return {
                    ...
                    case "screen_record": {
                        ...
                        const contentType = format === "mp4" ? "video/mp4" : undefined;
                        const outPath = typeof params.outPath === "string" && params.outPath.trim()
                            ? params.outPath.trim()
                            : "";
                        const fileName = outPath ? path.basename(outPath) : `screen-record.${format}`;
                        const saved = await saveMediaBuffer(buffer, contentType, "nodes", Math.max(buffer.length, 1), fileName);
                        const filePath = saved.path;
                        return {
                            content: [{ type: "text", text: `FILE:${filePath}` }],
                            details: {
