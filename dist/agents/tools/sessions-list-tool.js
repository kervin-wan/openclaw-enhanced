    ...
    return {
        ...
        execute: async (_toolCallId, args) => {
            ...
            for (const entry of sessions) {
                ...
                // Always show sessions spawned by the requester, regardless of
                // cross-agent policy. Sub-agents live in their own agent stores
                // and would otherwise be invisible to the spawning session.
                const spawnedByRequester = typeof entry.spawnedBy === "string"
                    && requesterInternalKey
                    && entry.spawnedBy === requesterInternalKey;
                if (crossAgent && !spawnedByRequester && !a2aPolicy.isAllowed(requesterAgentId, entryAgentId))
                    continue;
                if (key === "unknown")
                    continue;
                ...
                if (sessionId && storePath) {
                    try {
                        transcriptPath = resolveSessionFilePath(sessionId, 
