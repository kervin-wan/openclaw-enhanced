...
async function sweepSubagentRuns() {
    ...
    for (const [runId, entry] of subagentRuns.entries()) {
        // Enhanced: detect timed-out sub-agents
        if (!entry.endedAt && entry.startedAt && entry.runTimeoutMs && entry.runTimeoutMs > 0) {
            const elapsed = now - entry.startedAt;
            if (elapsed >= entry.runTimeoutMs + 30_000) {
                entry.endedAt = now;
                entry.outcome = { status: 'timeout', error: 'Timed out after ' + Math.floor(elapsed / 1000) + 's (limit: ' + Math.floor(entry.runTimeoutMs / 1000) + 's)' };
                mutated = true;
                const label = entry.label ? entry.label + ' ' : '';
                const requesterKey = entry.requesterSessionKey;
                if (requesterKey) {
                    try {
                        const { enqueueSystemEvent } = await import('../infra/system-events.js');
                        enqueueSystemEvent('⚠️ 子代理 ' + label + '超时（' + Math.floor(elapsed / 1000) + 's），任务可能未完成', { sessionKey: requesterKey });
                    } catch (_) { /* best effort */ }                }
                persistSubagentRuns();
            }
        }
...
export function registerSubagentRun(params) {
    ...
    subagentRuns.set(params.runId, {
        ...
        label: params.label,
        runTimeoutMs: params.runTimeoutSeconds ? params.runTimeoutSeconds * 1000 : undefined,
        createdAt: now,
        startedAt: now,
        archiveAtMs,
...
export function clearSubagentRunSteerRestart(runId) {
    const entry = subagentRuns.get(runId);
    if (entry) {
        delete entry["steerRestart"];
    }
}
export function markSubagentRunTerminated(runId) {
    const entry = subagentRuns.get(runId);
