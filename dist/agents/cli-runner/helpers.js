...
export async function cleanupResumeProcesses(backend, sessionId) {
    ...
    try {
        const stdout = await psWithFallback(["-axww", "-o", "pid=,ppid=,command="], ["-ax", "-o", "pid=,ppid=,command="]);
...
export async function cleanupSuspendedCliProcesses(backend, threshold = 10) {
    ...
    try {
        const stdout = await psWithFallback(["-axww", "-o", "pid=,ppid=,stat=,command="], ["-ax", "-o", "pid=,ppid=,stat=,command="]);
        const suspended = [];
        for (const line of stdout.split("\n")) {
            const trimmed = line.trim();
            if (!trimmed) {
