    ...
    try {
        ...
        try {
            ...
            try {
                ...
                // Enhanced: MicroCompact pre-step — clean old tool results before heavy compaction
                let microCompactBoundary = null;
                if (process.env.OPENCLAW_ENHANCED === '1') {
                    try {
                        const req = createRequire(import.meta.url);
                        const mc = req('../engine/IntegrationBridge.cjs').getMicroCompact();
                        if (mc) {
                            const mcResult = mc.compact(session.messages);
                            if (mcResult.stats.level !== 'none') {
                                log.info(`[Enhanced] MicroCompact pre-step: cleared ${mcResult.stats.cleared} old tool results, saved ~${mcResult.stats.tokensSaved} tokens`);
                                microCompactBoundary = mcResult.boundary;
                            }
                        }
                ...
                return {
                    ok: true,
                    compacted: true,
                // Enhanced: post-compaction MicroCompact + SnipCompact
                ...(process.env.OPENCLAW_ENHANCED === '1' ? (() => {
                  try {
                    const { createRequire } = require('node:module');
                    const req = createRequire(import.meta.url);
                    const rt = req('../../engine/EnhancedRuntime.cjs');
                    // MicroCompact: clean old tool results
                    const mcResult = rt.compactMessages(session.messages || []);
                    if (mcResult.boundary) {
                      log.info('[Enhanced] MicroCompact: ' + mcResult.stats.cleared + ' results cleared, ' + mcResult.stats.kept + ' kept');
                    }
                    // SnipCompact: check if aggressive compaction needed
                    const snipEngine = rt.getSnipCompact();
                    if (session.messages && snipEngine.needsSnip(session.messages)) {
                      log.warn('[Enhanced] SnipCompact: context large, consider manual /compact');
                    }
                    return { enhancedCompact: mcResult.stats };
                  } catch(e) { return {}; }
                })() : {}),
                    result: {
