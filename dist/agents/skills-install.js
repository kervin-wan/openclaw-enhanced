...
function formatScanFindingDetail(rootDir, finding) {
    ...
    const filePath = relativePath && relativePath !== "." && !relativePath.startsWith("..")
        ? relativePath
...
async function collectSkillInstallScanWarnings(entry) {
    ...
    try {
        ...
        if (summary.critical > 0) {
            ...
            warnings.push(`Skill "${skillName}" has ${summary.warn} suspicious code pattern(s). Run "clawdbot-cn security audit --deep" for details.`);
        }
    }
    catch (err) {
        warnings.push(`Skill "${skillName}" code safety scan failed (${String(err)}). Installation continues; run "clawdbot-cn security audit --deep" after install.`);
    }
    return warnings;
}
