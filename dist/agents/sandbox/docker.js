...
function formatSandboxRecreateHint(params) {
    if (params.scope === "session") {
        return formatCliCommand(`openclaw-enhanced sandbox recreate --session ${params.sessionKey}`);
    }
    if (params.scope === "agent") {
        const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
        return formatCliCommand(`openclaw-enhanced sandbox recreate --agent ${agentId}`);
    }
    return formatCliCommand("openclaw-enhanced sandbox recreate --all");
}
