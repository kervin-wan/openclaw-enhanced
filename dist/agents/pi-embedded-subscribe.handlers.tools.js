...
// Enhanced: lazy-load IntegrationBridge for PermissionFilter + HookSystem injections
let _enhancedBridge = null;
function getEnhancedBridge() {
    if (process.env.OPENCLAW_ENHANCED !== '1') return null;
    if (_enhancedBridge) return _enhancedBridge;
    try { const req = createRequire(import.meta.url); _enhancedBridge = req('../engine/IntegrationBridge.cjs'); }
    catch (_e) { _enhancedBridge = null; }
    return _enhancedBridge;
}
...
export async function handleToolExecutionStart(ctx, evt) {
    // Enhanced: PermissionFilter pre-check
    const bridge = getEnhancedBridge();
    if (bridge) {
        const permCheck = bridge.preToolCheck(String(evt.toolName), evt.args);
        if (!permCheck.allowed) {
            ctx.log.warn(`[Enhanced] Tool blocked by PermissionFilter: ${evt.toolName}`);
            throw new Error(`Tool ${evt.toolName} blocked: ${permCheck.reason || 'permission denied'}`);
        }
        // HookSystem: PreToolUse — fire and forget
        bridge.executeEventHooks('PreToolUse', { toolName: String(evt.toolName), args: evt.args }, ctx.params.sessionId)
            .catch(() => {});
    }
...
export async function handleToolExecutionEnd(ctx, evt) {
    ...
    // Enhanced: HookSystem PostToolUse / PostToolUseFailure
    const bridge = getEnhancedBridge();
    if (bridge) {
        const hookEvent = isToolError ? 'PostToolUseFailure' : 'PostToolUse';
        bridge.executeEventHooks(hookEvent, {
            toolName, toolCallId,
            isError: isToolError,
            result: sanitizedResult,
        }, ctx.params.sessionId).catch(() => {});
    }
