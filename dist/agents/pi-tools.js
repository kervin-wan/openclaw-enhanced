...
export function createOpenClawCodingTools(options) {
    ...
    const { agentId, globalPolicy, globalProviderPolicy, agentPolicy, agentProviderPolicy, profile, providerProfile, 
    // @ts-ignore -- cherry-pick upstream type mismatch
    // @ts-ignore -- cherry-pick upstream type mismatch
    profileAlsoAllow, 
    // @ts-ignore -- cherry-pick upstream type mismatch
    providerProfileAlsoAllow, } = resolveEffectiveToolPolicy({
    ...
    return process.env.OPENCLAW_ENHANCED === "1" ? _enhanced_shortenToolDescriptions(withAbort) : withAbort;
}



// Enhanced: shorten tool descriptions when pool > 20
function _enhanced_shortenToolDescriptions(tools) {
  ...
  try {
    ...
    if (ts.searchEnabled) {
      return tools.map(t => ({ ...t, description: (t.description || '').length > 60 ? (t.description || '').substring(0, 60) + '...' : (t.description || '') }));
    }
  } catch (e) {}
  return tools;
}
