...
export function resolveSubagentToolPolicy(cfg) {
    ...
    // Enhanced: check permission via PermissionFilter
    if (process.env.OPENCLAW_ENHANCED === '1') {
      try {
        const { createRequire } = require('node:module');
        const req = createRequire(import.meta.url);
        const rt = req('../../engine/EnhancedRuntime.cjs');
        const permResult = rt.checkPermission(toolName, 'default', [], input);
        if (permResult.denied) {
          return { allow: false, reason: permResult.reason };
        }
      } catch (e) { /* enhanced perm check unavailable */ }
    }
    return { allow, deny };
}
