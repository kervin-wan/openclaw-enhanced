...
// Enhanced: lazy-load runtime patches for guidance injection
let _enhancedPatches = null;
function getEnhancedPatches() {
    if (process.env.OPENCLAW_ENHANCED !== '1') return null;
    if (_enhancedPatches) return _enhancedPatches;
    try {
        const req = createRequire(import.meta.url);
        _enhancedPatches = req('../engine/runtime-patches.cjs');
    } catch (_e) { _enhancedPatches = null; }
    return _enhancedPatches;
}

/** Enhanced: insert cache boundary and compact-awareness markers plus guidance */
function _enhancePrompt(prompt, tools) {
    const boundaryAnchor = "## Workspace Files (injected)";
    const cacheBoundary = "<!-- PROMPT_CACHE_BOUNDARY -->";
    const compactNote = "<!-- [Enhanced] Context management: tool results may be cleaned up. Write important info in your response. -->";
    
    const lines = prompt.split('\n');
    let boundaryIdx = -1;
    for (let i = 0; i < lines.length; i++) {
        if (lines[i].trim() === boundaryAnchor || lines[i].startsWith(boundaryAnchor)) {
            boundaryIdx = i;
            break;
        }
    }
    
    if (boundaryIdx === -1) return prompt;
    
    const staticPart = lines.slice(0, boundaryIdx);
    const dynamicPart = lines.slice(boundaryIdx);
    
    const cacheKey = crypto.createHash('sha256')
        .update(staticPart.join('\n'))
        .digest('hex')
        .substring(0, 12);
    
    const patches = getEnhancedPatches();
    let enhancedSections = [];
    if (patches) {
        const tsGuidance = tools ? patches.buildToolSearchGuidance(tools) : null;
        if (tsGuidance) enhancedSections.push(tsGuidance);
        const coordGuidance = patches.buildCoordinatorGuidance();
        if (coordGuidance) enhancedSections.push(coordGuidance);
        const mcpGuidance = patches.buildMCPGuidance();
        if (mcpGuidance) enhancedSections.push(mcpGuidance);
        const agentGuidance = patches.buildAgentTypeGuidance();
        if (agentGuidance) enhancedSections.push(agentGuidance);
        const rbGuidance = patches.buildRemoteBridgeGuidance();
        if (rbGuidance) enhancedSections.push(rbGuidance);
        const sdkGuidance = patches.buildSDKLayerGuidance();
        if (sdkGuidance) enhancedSections.push(sdkGuidance);
    }
    
    return [
        ...staticPart,
        '',
        `${cacheBoundary}<!-- cache_key=${cacheKey} -->`,
        compactNote,
        ...enhancedSections,
        '',
        ...dynamicPart,
...
export function buildAgentSystemPrompt(params) {
    // Enhanced: wrap with cache boundary + compact awareness + guidance
    if (process.env.OPENCLAW_ENHANCED === '1' && params._enhanced !== false) {
        try {
            const prompt = _buildAgentSystemPromptImpl(params);
            const toolNames = params.availableTools
                ? Array.from(params.availableTools)
                : (params.toolNames || []);
            return _enhancePrompt(prompt, toolNames);
        } catch (_e) { /* fall through to original */ }
    }
    return _buildAgentSystemPromptImpl(params);
}

function _buildAgentSystemPromptImpl(params) {
