...
import { buildGroupDisplayName, canonicalizeMainSessionAlias, loadSessionStore, resolveFreshSessionTotalTokens, resolveMainSessionKey, resolveStorePath, } from "../config/sessions.js";
...
function listConfiguredAgentIds(cfg) {
    ...
    if (agents.length > 0) {
        ...
        ids.add(defaultId);
        // Also discover agents that exist on disk (e.g. explore/review stores
        // created by sessions_spawn) so sub-agent sessions are visible in
        // sessions_list even when they aren't explicitly configured.
...
function findStoreKeysIgnoreCase(store, needle) {
