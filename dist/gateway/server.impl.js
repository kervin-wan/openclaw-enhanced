...
import { registerSkillsChangeListener, ensureSkillsWatcher, } from "../agents/skills/refresh.js";
...
export async function startGatewayServer(port = 18789, opts = {}) {
    ...
    if (migrationResult.migrated) {
        log.info(`gateway: migrated workspace directory from ${migrationResult.legacyDir} to ${migrationResult.newDir}`);
    }
    ...
    if (configSnapshot.legacyIssues.length > 0) {
        ...
        if (!migrated) {
            throw new Error(`Legacy config entries detected but auto-migration failed. Run "${formatCliCommand("openclaw-enhanced doctor")}" to migrate.`);
        }
    ...
    if (configSnapshot.exists && !configSnapshot.valid) {
        ...
        throw new Error(`Invalid config at ${configSnapshot.path}.\n${issues}\nRun "${formatCliCommand("openclaw-enhanced doctor")}" to repair, then retry.`);
    }
  ...
  // Enhanced: full IntegrationBridge initialization (all 22 engine modules)
  let enhancedBridge = null;
  let enhancedPatches = null;
  if (process.env.OPENCLAW_ENHANCED === '1') {
    try {
      const { createRequire: cr2 } = await import('node:module');
      const req2 = cr2(import.meta.url);
      const bridge = req2('../engine/IntegrationBridge.cjs');
      const initResult = bridge.initAll({ log });
      enhancedBridge = bridge;
      log.info(`[Enhanced] IntegrationBridge: ${initResult.initialized.length} modules initialized, ${initResult.skipped.length} skipped`);
      if (initResult.errors.length > 0) {
        log.warn(`[Enhanced] init errors: ${initResult.errors.join('; ')}`);
      }
      // Load runtime patches for message compression + tool search + fork agent
      try {
        enhancedPatches = req2('../engine/runtime-patches.cjs');
        log.info('[Enhanced] Runtime patches loaded');
      } catch (e2) {
        log.warn('[Enhanced] Runtime patches skipped: ' + e2.message);
      }
      // Load gateway methods (Coordinator + RemoteBridge + SDKLayer) + enable feature flags
      try {
        const gwMethods = req2('../engine/gateway-methods.cjs');
        const flags = gwMethods.enableAllFeatureFlags();
        log.info(`[Enhanced] Gateway methods loaded, flags: coordinator=${flags.coordinator}, remote_bridge=${flags.remote_bridge}, sdk_layer=${flags.sdk_layer}`);
      } catch (e3) {
        log.warn('[Enhanced] Gateway methods skipped: ' + e3.message);
      }
    } catch (e) {
      log.warn('[Enhanced] IntegrationBridge init failed: ' + e.message);
    }
  }
  ...
  // Enhanced: session lifecycle hooks — called when sessions are created/destroyed
  const enhancedSessionHooks = enhancedBridge ? {
    onStart: (sessionId, ctx) => enhancedBridge.onSessionStart(sessionId, ctx),
    onEnd: (sessionId, ctx) => enhancedBridge.onSessionEnd(sessionId, ctx),
  ...
  const nodeSendEvent = (opts) => {
    ...
    async function handleSkillsChangeForSecurityHook(event) {
        ...
        let skillsFileDir = null;
        let skillsPackageDirName = null;
        let skillsName = null;
        ...
        if (skillsFileDir) {
            try {
                ...
                const sourceLabel = root === workspaceSkillsRoot
                    ? "openclaw-workspace"
        ...
        if (skillsFileDir && skillsName) {
            ...
            runner
                ...
                void skillsHandlers["skills.update"]({
                    ...
                    respond: (ok, error) => {
                        if (!ok) {
                            logHooks.error(`[skills.update] skillsName: "${skillsName}" , failed: ${JSON.stringify(error ?? {})}}`);
                        }
                    },
                    context: ctx,
                });
