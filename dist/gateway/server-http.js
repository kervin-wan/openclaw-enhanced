...
export function createGatewayHttpServer(opts) {
    const { canvasHost, clients, controlUiEnabled, controlUiBasePath, openAiChatCompletionsEnabled, openResponsesEnabled, openResponsesConfig, handleHooksRequest, handlePluginRequest, resolvedAuth, } = opts;
    ...
    async function handleRequest(req, res) {
        ...
        try {
            ...
            // ├ Enhanced: Thinking state endpoint (reads/writes thinking.conf)
            if (requestPath === '/thinking/state') {
                const { createRequire: cr3 } = await import('node:module');
                const req3 = cr3(import.meta.url);
                const pathMod = req3('path');
                const fs = req3('fs');
                const os = req3('os');
                const thinkingConf = pathMod.join(os.homedir(), '.openclaw-enhanced', 'thinking.conf');
                if (req.method === 'GET') {
                    try {
                        const val = fs.readFileSync(thinkingConf, 'utf-8').trim() || 'off';
                        sendJson(res, 200, { mode: val });
                    } catch (_) { sendJson(res, 200, { mode: 'off' }); }
                    return;
                }
                if (req.method === 'POST') {
                    const body = await readJsonBody(req, 1024);
                    if (!body.ok) { sendJson(res, 400, { ok: false, error: body.error }); return; }
                    const mode = body.value?.mode;
                    if (!mode || !['off','auto','manual'].includes(mode)) {
                        sendJson(res, 400, { ok: false, error: 'invalid mode: use off/auto/manual' }); return;
                    }
                    try {
                        fs.writeFileSync(thinkingConf, mode + '\n', 'utf-8');
            ...
            // Enhanced: Full dashboard page
            if (requestPath === '/enhanced/dashboard') {
                const { createRequire: crDash } = await import('node:module');
                const reqDash = crDash(import.meta.url);
                const htmlPath = reqDash.resolve('../control-ui/enhanced-dashboard.html');
                const fs = reqDash('fs');
                try {
            ...
            // Enhanced: Full status endpoint (features, model quality, hooks)
            if (requestPath === '/enhanced/status') {
                try {
                    const { createRequire: crStat } = await import('node:module');
                    const reqStat = crStat(import.meta.url);
                    const ib = reqStat('../engine/IntegrationBridge.cjs');
                    sendJson(res, 200, ib.getStatus());
                } catch (e) {
                    sendJson(res, 500, { error: 'enhanced status unavailable', detail: e.message });
                }
                return;
            }
            // Enhanced: Hot reload endpoint
            if (requestPath === '/enhanced/reload') {
                if (req.method !== 'POST') {
                    res.statusCode = 405;
                    res.setHeader('Allow', 'POST');
                    res.end('Method Not Allowed');
                ...
                try {
                    const { createRequire: crReload } = await import('node:module');
                    const reqReload = crReload(import.meta.url);
                    const ib = reqReload('../engine/IntegrationBridge.cjs');
                    const result = ib.resetAll();
