...
export function createAgentEventHandler({ broadcast, nodeSendToSession, agentRunSeq, chatRunState, resolveSessionKeyForRun, clearAgentRunContext, enhancedSessionHooks, }) {
    ...
    // Enhanced: track started sessions for lifecycle hooks
    const startedSessions = new Set();
    const triggerSessionStart = (sessionKey) => {
        if (!enhancedSessionHooks?.onStart || !sessionKey) return;
        if (startedSessions.has(sessionKey)) return;
        startedSessions.add(sessionKey);
        enhancedSessionHooks.onStart(sessionKey, { timestamp: Date.now() });
    };
    const triggerSessionEnd = (sessionKey) => {
        if (!enhancedSessionHooks?.onEnd || !sessionKey) return;
        enhancedSessionHooks.onEnd(sessionKey, { timestamp: Date.now() });
    ...
    return (evt) => {
        ...
        // Enhanced: trigger session lifecycle hooks
        if (sessionKey) {
            if (lifecyclePhase === "start" || (!lifecyclePhase && !startedSessions.has(sessionKey))) {
                triggerSessionStart(sessionKey);
            }
        }
        if (sessionKey) {
        ...
        if (lifecyclePhase === "end" || lifecyclePhase === "error") {
            ...
            // Enhanced: trigger session end hook
            triggerSessionEnd(sessionKey);
        }
    };
}
